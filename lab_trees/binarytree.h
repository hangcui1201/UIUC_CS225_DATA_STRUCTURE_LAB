...
class BinaryTree
{
    ...
    private:
        ...
        // Declare the recursive helper functions below

        void mirrorHelperFunction(Node* subRoot);

        bool isOrderedRecursiveHelperFunction(const Node* subRoot) const;
        T maxValueInLeftSubTree(const Node* subRoot) const;
        T minValueInRightSubTree(const Node* subRoot) const;

        void getPathsHelperFunction(const Node* subRoot, std::vector<T> path, std::vector<std::vector<T>> &paths) const;

        // int sumDistances() const;
        int sumDistancesHelperFunction(const Node* subRoot, int count) const;
