...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // one of the queues is empty and one has a single node
    while (! (singleQueue.size() + mergeQueue.size() == 1)) {
        ...
        TreeNode * parentNode = new TreeNode(smallest_1->freq.getFrequency() + smallest_2->freq.getFrequency());

        parentNode->left = smallest_1;
        parentNode->right = smallest_2;

        mergeQueue.push(parentNode);

    }

    // Only one node left either in the singeQueue or mergeQueue
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        // Leaf node
        if (bfile.getNextBit()) {
            Frequency frequency(bfile.getNextByte(), 0);
            TreeNode * leafNode = new TreeNode(frequency);
            return leafNode;
        } else {
