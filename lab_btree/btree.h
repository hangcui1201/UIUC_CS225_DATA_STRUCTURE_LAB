...
size_t binary_search(const std::vector<T>& elements, int begin, int end, const C& val)
{
    // Only two elements left
    if (begin == end-1 && val>elements[begin] && val<elements[end]){
        return end;
    }
    
    size_t mid_idx = (begin + end) / 2;

    if (elements[mid_idx] == val) {
        return mid_idx;
    }

    if (elements[mid_idx] < val) {
        return binary_search(elements, mid_idx, end, val);
    }
        
    if (elements[mid_idx] > val) {
        return binary_search(elements, begin, mid_idx, val);
    }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    int vec_size = elements.size();

    if (vec_size == 0 || val < elements[0]) {
        return 0;
    }

    if (val > elements[vec_size-1]){
        return vec_size;
    } 
    ...
    return binary_search(elements, 0, vec_size-1, val);
