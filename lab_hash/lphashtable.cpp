...
void LPHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    size_t newTableSize = findPrime(size*2);
    std::pair<K, V>** newtable = new std::pair<K, V>*[newTableSize];

    delete[] should_probe;
    should_probe = new bool[newTableSize];
    ...
    for (size_t i = 0; i < size; i++ ) {
        if(table[i] != NULL) { 
            size_t idx = hashes::hash(table[i]->first, newTableSize);
            while (should_probe[idx]) {
                idx = (idx+1) % newTableSize;
            }
